...
import { TableColumns } from '@/components/purchase-order/TableColumns';
import { PurchaseOrderTableProps } from '@/types/purchaseOrder';
...
export const TablePurchaseOrder = ({
  ...
  const columns = useMemo(
    () =>
      TableColumns(onEdit, onDelete, onView, onStatusFilter, onCreatorFilter),
    [onEdit, onDelete, onView, onStatusFilter, onCreatorFilter]
  );
        ...
        <div className="w-full min-w-max bg-white text-left text-slate-700">
              ...
              {table.getHeaderGroups().map((headerGroup) =>
                headerGroup.headers.map((header) => (
                  <div
                    key={header.id}
                    className={`${
                      header.id === 'created_by' ? 'flex-grow' : 'flex'
                    } flex-shrink-0`}
                  >
                    {flexRender(
          ...
          {isLoading ? (
            ...
            <EmptyState
              ...
              description="Try adding a new purchase order or adjusting your search."
            />
          ) : (
            <div
              style={{
                  ...
                  >
                    {row.getVisibleCells().map((cell) => (
                      <div
                        key={cell.id}
                        className={`${
                          cell.column.id === 'created_by' ? 'flex-grow' : 'flex'
                        } flex-shrink-0`}
                      >
                        {flexRender(
