...
import { TableColumns } from '@/components/receive/TableColumns';
import { ReceiveTableProps } from '@/types/receive';
...
export const TableReceive = ({
  ...
  const columns = useMemo(
    ...
    [onEdit, onDelete, onView, onReceiverFilter, onSupplierFilter]
  );
      ...
      >
        <div className="w-full min-w-max bg-white text-left text-slate-700">
              ...
              {table.getHeaderGroups().map((headerGroup) =>
                headerGroup.headers.map((header) => (
                  <div
                    key={header.id}
                    className={`${
                      header.id === 'receive_by' ? 'flex-grow' : 'flex'
                    } flex-shrink-0`}
                  >
                    {flexRender(
          ...
          {isLoading ? (
            ...
            <EmptyState
              title="No Receive Records Found"
              description="Try adding a new receive record or adjusting your search."
            />
          ) : (
            <div
              style={{
                  ...
                  >
                    {row.getVisibleCells().map((cell) => (
                      <div
                        key={cell.id}
                        className={`${
                          cell.column.id === 'receive_by' ? 'flex-grow' : 'flex'
                        } flex-shrink-0`}
                      >
                        {flexRender(
