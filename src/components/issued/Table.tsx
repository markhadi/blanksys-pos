...
import { TableColumns } from '@/components/issued/TableColumns';
import { IssuedTableProps } from '@/types/issued';

export const TableIssued = ({
  data,
  isLoading,
  onEdit,
  onDelete,
  ...
  const columns = useMemo(
    () => TableColumns(onEdit, onDelete, onView, onIssuedByFilter),
    [onEdit, onDelete, onView, onIssuedByFilter]
  );
        ...
        <div className="w-full min-w-max bg-white text-left text-slate-700">
              ...
              {table.getHeaderGroups().map((headerGroup) =>
                headerGroup.headers.map((header) => (
                  <div
                    key={header.id}
                    className={`${
                      header.id === 'issuedBy' ? 'flex-grow' : 'flex'
                    } flex-shrink-0`}
                  >
                    {flexRender(
          ...
          {isLoading ? (
            ...
            <EmptyState
              ...
              description="Try adding a new issued record or adjusting your search."
            />
          ) : (
            <div
              style={{
                  ...
                  >
                    {row.getVisibleCells().map((cell) => (
                      <div
                        key={cell.id}
                        className={`${
                          cell.column.id === 'issuedBy' ? 'flex-grow' : 'flex'
                        } flex-shrink-0`}
                      >
                        {flexRender(
